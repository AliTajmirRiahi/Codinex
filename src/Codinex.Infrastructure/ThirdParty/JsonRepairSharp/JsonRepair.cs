// Vendored from https://github.com/thijse/JsonRepairSharp (MIT License), itself a port of
// https://github.com/josdejong/jsonrepair. Ported here because the upstream NuGet package
// (JsonRepairSharp) only targets net8.0/net9.0/net10.0 and cannot be referenced from this
// net472 project. Adapted to compile without C# implicit usings / target-framework extras
// (Dictionary.GetValueOrDefault isn't available on net472, so it is replaced with TryGetValue).
            ...
            if (StringUtils.IsQuote(_text.CharCodeAt(_i)))
            {
                ...
                while (_i < _text.Length && !MatchingQuotes.IsMatchingEndQuote(_text.CharCodeAt(_i)))
                {
                    if (_text.CharCodeAt(_i) == StringUtils.CodeBackslash)
                    {
                        ...
                        string escapeChar;
                        EscapeCharacters.TryGetValue(character, out escapeChar);
                        if (escapeChar != null)
                        {
