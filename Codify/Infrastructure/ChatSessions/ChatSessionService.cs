...
namespace Codify.Infrastructure.ChatSessions
{
    public class ChatSessionService
    {
        ...
        public ChatSessionService(ChatManager chatManager, ProviderManager providerManager)
        {
        ...
        {
            var chats = await _chatManager.GetAllChatsAsync();
            ...
            var providerId = _providerManager.ActiveProvider?.Id ?? throw new InvalidOperationException("No active provider found.");
            var modelId = _providerManager.ActiveModel?.Id ?? throw new InvalidOperationException("No active model found.");
        ...
        {
            var doc = await _chatManager.CreateChatAsync(providerId, modelId);
        ...
        {
            var session = new ChatSession(_chatManager);
